...
int main() {
    ...
    outputFile << setprecision(6) << fixed;
    ...
    while (getline(inputFile, line)) {
        ...
        if (instruction == 1) {
            ...
            logType = log.substr(1, pos-1);
            log = log.substr(pos+1);
            istringstream(log) >> severity;

            monitor.insertLog(timestamp, logType, severity);
            outputFile << "No output" << endl;
            
        } else if (instruction == 2) {
            string logType;
            ss >> logType; 
            auto result = monitor.computeLogTypeStats(logType);
            outputFile << "Min: " << get<0>(result) << ", Max: " << get<1>(result) << ", Mean: " << get<2>(result) << endl;
            ...
            ss >> timestamp;
            if (window == "BEFORE") {
                auto result = monitor.computeBeforeTimestampStats(timestamp);
                outputFile << "Min: " << get<0>(result) << ", Max: " << get<1>(result) << ", Mean: " << get<2>(result) << endl;
            } else {
                auto result = monitor.computeAfterTimestampStats(timestamp);
                outputFile << "Min: " << get<0>(result) << ", Max: " << get<1>(result) << ", Mean: " << get<2>(result) << endl;
            }
            ...
            ss >> timestamp;
            if (window == "BEFORE") {
                auto result = monitor.computeBeforeTimestampLogTypeStats(logType, timestamp);
                outputFile << "Min: " << get<0>(result) << ", Max: " << get<1>(result) << ", Mean: " << get<2>(result) << endl;
            } else {
                auto result = monitor.computeAfterTimestampLogTypeStats(logType, timestamp);
                outputFile << "Min: " << get<0>(result) << ", Max: " << get<1>(result) << ", Mean: " << get<2>(result) << endl;
            }
